import { StyleSheet, Text, SafeAreaView, Pressable, Image, FlatList, View, Component } from "react-native";
...
<View>
    ...
    {/* <View>
    <Text style={{color: Colors.white}}>{JSON.stringify(tracks[0].external_urls.spotify)}</Text>
    </View> */}
    <FlatList
    data={tracks}
    renderItem={({ item, index }) => renderSong(item, index)}
    keyExtractor={(item) => {return JSON.stringify(item.index);}} />
</View>
